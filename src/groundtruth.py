...
def ranksumtest_for_groundtruth(adata_adt, top_proteins = False): #top_proteins = True then we will jsut take the top 20 ADTs
    rows = []
    ...
    surface_protein_markeer_df = surface_protein_markeer_df[(surface_protein_markeer_df["Score_Wilcox_Ranksum"] > 0) & (surface_protein_markeer_df["Mean_Difference"] > 0)].copy()
    ...
    return surface_protein_markeer_df
...
def create_groundtruth(adt_dataset):
    ...
    ground_truth_df = search_uniprot_protein_to_gene(top_surface_proteins_with_ranksum["Surface_Protein"].unique())
    gt_merged = top_surface_proteins_with_ranksum[["Celltype", "Surface_Protein"]].drop_duplicates().merge(ground_truth_df[["Surface_Protein", "Gene Names"]],on="Surface_Protein",how="left")
